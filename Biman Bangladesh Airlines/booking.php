...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['continue'])) {
    // Escape user inputs to protect from SQL injection
    ...
    if ($stmt->execute()) {
        $booking_id = $conn->insert_id;  // retrieve the last insert id for the booking
...
<body>
        ...
        <div class="container">
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav mx-auto">
                    <li class="nav-item">
                        <span class="nav-text">Review Your Booking</span> <!-- Changed to nav-text for clarity -->
                    </li>
                </ul>
            </div>
        </div>
    ...
    <script>
        // JavaScript to toggle the visibility of the extra details
        function toggleDetails(id) {
            const extraDetails = document.getElementById(id);
            if (extraDetails.style.display === "none" || extraDetails.style.display === "") {
                extraDetails.style.display = "block"; // Show the extra details
            } else {
                extraDetails.style.display = "none"; // Hide the extra details
            }
        }
    ...
    // Calculate maximum date for DOB (12 years ago)
    ...
    // Calculate minimum date for Passport Expiry (6 months from today)
